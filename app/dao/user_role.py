...
class UserRoleDAO(BaseDAO[UserRole]):
    ...
    async def bind_roles(self, user_id: int, role_ids: Sequence[int]) -> dict[str, int]:
        ...
        existed_active: set[int] = {int(x.role_id) for x in existing if not getattr(x, "is_deleted", False)}
        to_create = [
            rid for rid in role_ids if rid not in existed_active and all(int(x.role_id) != rid for x in existing)
        ]
    ...
    async def bind_roles_to_users(self, user_ids: Sequence[int], role_ids: Sequence[int]) -> dict[str, int]:
        ...
        for x in existing:
            exist_map[(int(x.user_id), int(x.role_id))] = (int(x.id), bool(getattr(x, "is_deleted", False)))
